# output: CLANG_FOUND CLANG_INCLUDE_DIRS CLANG_DEFINITIONS CLANG_EXECUTABLE
...
function(is_clang_installed config_cmd)
    execute_process(COMMAND ${config_cmd} --includedir
                    OUTPUT_VARIABLE include_dirs OUTPUT_STRIP_TRAILING_WHITESPACE)
    execute_process(COMMAND ${config_cmd} --src-root
                    OUTPUT_VARIABLE llvm_src_dir OUTPUT_STRIP_TRAILING_WHITESPACE)
...
function(set_clang_include_dirs config_cmd)
    ...
    if(CLANG_INSTALLED)
        ...
        execute_process(COMMAND ${config_cmd} --src-root
                        OUTPUT_VARIABLE llvm_src_dir OUTPUT_STRIP_TRAILING_WHITESPACE)
        execute_process(COMMAND ${config_cmd} --obj-root
                        OUTPUT_VARIABLE llvm_obj_dir OUTPUT_STRIP_TRAILING_WHITESPACE)
...
function(set_clang_ldflags config_cmd)
    is_clang_installed(${config_cmd})
    if(CLANG_INSTALLED)
        execute_process(COMMAND ${config_cmd} --ldflags
                        OUTPUT_VARIABLE ldflags OUTPUT_STRIP_TRAILING_WHITESPACE)
    endif()
...
find_program(
    LLVM_CONFIG
    NAMES llvm-config llvm-config-10 llvm-config-9 llvm-config-8 llvm-config-64
    PATHS ENV LLVM_PATH)
if(LLVM_CONFIG)
...
message(STATUS "llvm-config filtered link flags : ${CLANG_LDFLAGS}")
